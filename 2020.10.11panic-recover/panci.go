...
func main()  {
	...
	panic	:词义"恐慌"
	recover	:"恢复"
	go语言利用panci(),recover(),实现程序中的极特殊的异常的处理
		panic(),让当前程序进入恐慌,中断程序的执行
		recover(),让程序恢复，必须在defer函数中执行
	 */
	funA()
	defer myprinc("defer main:3....")
	funB()
	defer myprinc("defer main:4....")

	fmt.Println("main..over....")
}

func myprinc(s string){
	fmt.Println(s)
}
